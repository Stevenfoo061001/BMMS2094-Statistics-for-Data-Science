...
save_plot(autoplot(fit) + autolayer(test_ts, series = "Actual test CPI") + labs(title = "Tan Wei Ching: Trend and Monthly-Seasonality 18-Month Holdout Forecast", x = "Year", y = "CPI index") + theme_minimal(), file.path(out, "Member_D_02_forecast.png"))
...
accuracy_D <- metrics(test$index, as.numeric(fit$mean), train$index) %>%
  ...
  mutate(member = "Tan Wei Ching", model = "Trend + Monthly Dummies") %>% select(member, model, everything())
