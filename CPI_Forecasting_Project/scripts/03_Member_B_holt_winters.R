...
save_plot(autoplot(fit) + autolayer(test_ts, series = "Actual test CPI") + labs(title = "Ooi Mei Yi: Holt-Winters 18-Month Holdout Forecast", x = "Year", y = "CPI index") + theme_minimal(), file.path(out, "Member_B_03_forecast.png"))
...
accuracy_B <- metrics(test$index, as.numeric(fit$mean), train$index) %>%
  ...
  mutate(member = "Ooi Mei Yi", model = "Holt-Winters") %>% select(member, model, everything())
