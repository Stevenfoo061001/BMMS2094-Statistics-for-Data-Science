...
save_plot(autoplot(fit) + autolayer(test_ts, series = "Actual test CPI") + labs(title = "Huxley: Seasonal Naive 12-Month Forecast", x = "Year", y = "CPI index") + theme_minimal(), file.path(out, "Member_A_03_forecast.png"))
accuracy_A <- metrics(test$index, as.numeric(fit$mean), train$index) %>% mutate(member = "Huxley", model = "Seasonal Naive") %>% select(member, model, everything())
