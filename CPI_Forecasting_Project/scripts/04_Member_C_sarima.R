...
save_plot(autoplot(fit) + autolayer(test_ts, series = "Actual test CPI") + labs(title = "Steven: SARIMA 18-Month Holdout Forecast", x = "Year", y = "CPI index") + theme_minimal(), file.path(out, "Member_C_04_forecast.png"))
png(file.path(out, "Member_C_05_residuals.png"), width = 3300, height = 2400, res = 300); checkresiduals(fit_model); dev.off()
accuracy_C <- metrics(test$index, as.numeric(fit$mean), train$index) %>%
  bind_cols(ljung_box_diagnostic(fit_model, fitdf = length(coef(fit_model)))) %>%
  mutate(member = "Steven", model = "SARIMA(1,1,1)(1,1,1)[12]") %>% select(member, model, everything())
